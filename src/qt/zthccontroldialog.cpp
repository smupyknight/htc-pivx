...
#include "ui_zthccontroldialog.h"
...
std::list<std::string> ZThcControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZThcControlDialog::listMints;

ZThcControlDialog::ZThcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZThcControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZThcControlDialog::~ZThcControlDialog()
{
    delete ui;
}

void ZThcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZThcControlDialog::updateList()
{
...
void ZThcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZThcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZThc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZThcControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZThcControlDialog::GetSelectedMints()
{
...
void ZThcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
