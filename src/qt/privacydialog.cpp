                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTHC ought to be enough for anybody." - Bill Gates, 2017
    ui->zTHCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTHCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTHC->setEnabled(false);
        ui->pushButtonMintzTHC->setToolTip(tr("zTHC is currently disabled due to maintenance."));

        ui->pushButtonSpendzTHC->setEnabled(false);
        ui->pushButtonSpendzTHC->setToolTip(tr("zTHC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTHCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTHC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTHC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTHC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTHC in ") + 
...
void PrivacyDialog::on_pushButtonSpendzTHC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTHC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTHC
        sendzTHC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTHC
    sendzTHC();
}

void PrivacyDialog::on_pushButtonZThcControl_clicked()
{
    ZThcControlDialog* zThcControl = new ZThcControlDialog(this);
    zThcControl->setModel(walletModel);
    zThcControl->exec();
}

void PrivacyDialog::setZThcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzThcSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTHC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Thc Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTHCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTHC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " THC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTHC</b>";
    ...
    // use mints from zThc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZThcControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZThcControlDialog::GetSelectedMints();
    }
    ...
    // Clear zthc selector in case it was used
    ZThcControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zThc Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Thc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zThc Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTHC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTHC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTHC "));
    ui->labelzTHCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTHCSyncStatus->setVisible(fShow);
}
