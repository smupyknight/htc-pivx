...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a THC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no THC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert thc:// to thc:
    //
    //    Cannot handle this later, because thc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open thc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "THC.lnk";
}
...
{
    return GetAutostartDir() / "thc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a thc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the thc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add thc app to startup item list
